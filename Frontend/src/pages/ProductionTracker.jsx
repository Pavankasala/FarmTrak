  ...
  return (
    ...
    >
      <PageHeader
        ...
        description="Monitor and record daily egg production across all your flocks"
      />
      ...
      <motion.form onSubmit={handleSubmit} className="w-full max-w-4xl glass-effect rounded-3xl p-8 shadow-xl border border-white/20 dark:border-slate-700/50" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
        <h2 className="text-2xl font-semibold text-slate-800 dark:text-slate-100 mb-6">{form.id ? "Update Production Record" : "Record Egg Production"}</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <div className="space-y-2">
            <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Select Flock</label>
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Egg Count</label>
            <input type="number" name="count" value={form.count} onChange={handleChange} placeholder="Number of eggs" className={inputStyle}/>
          </div>
          <div className="space-y-2">
            <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Collection Date</label>
